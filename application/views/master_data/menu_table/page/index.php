      ...
      <tbody>
        <?php foreach ($data['data'] as $key => $res) : ?>
          <tr>
            <td><?= $key + 1; ?></td>
            <td>
              <a href="<?= base_url('master_data/menu_table/edit/' . $res['id']); ?>" class="btn btn-primary btn-sm">Ubah</a>
              <a href="<?= base_url('master_data/menu_table/delete/' . $res['id']); ?>" class="btn btn-danger btn-sm btn-del">Hapus</a>
            </td>
            <td><?= $res['no_meja']; ?></td>
